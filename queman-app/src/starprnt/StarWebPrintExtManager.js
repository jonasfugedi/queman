...
StarWebPrintExtManager.prototype.connect = function (a) {
    void 0 != a && (void 0 != a.pollingTimeout && (this.pollingTimeout = a.pollingTimeout), void 0 != a.pollingInterval && (this.pollingInterval = a.pollingInterval), void 0 != a.url && (this.url = a.url));
    return !this.isPolling && !this.isWaitCallback ? (this.isReleaseRequest = !1, this._sendExtMessage({
        requestType: "claim",
...
StarWebPrintExtManager.prototype._polling = function () {
    this.isReleaseRequest ? (this.isReleaseRequest = !1, this._sendExtMessage({
        requestType: "release",
        isNeedExternalCallBack: !0
    }), this._clearClaimId()) : this._sendExtMessage({requestType: "read", isNeedExternalCallBack: !1})
};
StarWebPrintExtManager.prototype._analyzeXml = function (a, d) {
    var e = new DOMParser, b, f;
    ...
    var g = b.getElementsByTagName("claim").item(0), e = b.getElementsByTagName("event");
    ...
    if ("true" == a.managerSuccess) {
        if ("true" == a.managerClaim) {
            void 0 != f && (this.claimId = f.textContent, this.isPolling = !0);
            var c = this;
            f = {
                ...
                }, onDisplayImpossible: function () {
                    c.onDisplayImpossible()
                }, onDisplayConnect: function () {
                    c.onDisplayConnect()
                },
            ...
            if (this.isPolling || this.isWaitWriteCallback) {
                if (void 0 != b.getElementsByTagName("eventcount")) try {
                    g = b.getElementsByTagName("eventcount").item(0).textContent
                } catch (m) {
                }
                for (b =
                         0; b < g; b++) for (var h = 0; h < e.length; h++) {
                    var k = e.item(h).getElementsByTagName("action").item(0).textContent;
                    if (e.item(h).getElementsByTagName("number").item(0).textContent == b) if ("onBarcodeDataReceive" == k || "onStatusUpdate" == k) {
                        var l = e.item(h).getElementsByTagName("data").item(0).textContent;
                        try {
                            f[k](l)
                        } catch (n) {
                        }
    ...
    } else 1200 != a.managerCode && (2001 != a.managerCode && !(d && 1010 == a.managerCode)) && this._clearClaimId();
    return a
};
...
StarWebPrintExtManager.prototype._sendExtMessage = function (a) {
    ...
    var d = "<extmanager ";
    "claim" == a.requestType || "read" == a.requestType || "write" == a.requestType ? (d += 'request_type="' + a.requestType + '" ', "claim" == a.requestType && (d += 'polling_timeout="' + this._getPollingTimeout() + '" ')) : d += 'request_type="release" ';
    0 != this.claimId && (d += 'claimid="' + this.claimId + '" ');
    d += ">";
    "write" == a.requestType && void 0 != a.request &&
    (d += a.request);
    var d = d + "</extmanager>", e = new StarWebPrintTrader, b = this, f = this._getPollingTimeout(),
        g = "write" == a.requestType;
    g ? (e.onReceive = function (c) {
        c = b._analyzeXml(c, g);
        if (a.isNeedExternalCallBack || "false" == c.managerSuccess) {
            if (void 0 != b.onReceive) b.onReceive(c);
